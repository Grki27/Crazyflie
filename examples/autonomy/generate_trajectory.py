...
def generate_trajectory(data, num_pieces):
  ...
  resX = scipy.optimize.minimize(func, x0, (data[:,0], data[:,1], piece_length), method="SLSQP", options={"maxiter": 10}, 
    constraints=constraints
    )
  resY = scipy.optimize.minimize(func, x0, (data[:,0], data[:,2], piece_length), method="SLSQP", options={"maxiter": 10}, 
    constraints=constraints
    )
  resZ = scipy.optimize.minimize(func, x0, (data[:,0], data[:,3], piece_length), method="SLSQP", options={"maxiter": 10}, 
    constraints=constraints
    )

  resYaw = scipy.optimize.minimize(func, x0, (data[:,0], data[:,4], piece_length), method="SLSQP", options={"maxiter": 10}, 
    constraints=constraints
    )
...
if __name__ == "__main__":
    # Definiranje točaka kvadratne putanje (vrijeme, x, y, z, yaw)
    data = np.array([
        [0, 0, 0, 0, 0],   # Početak
        [1, 1, 1, 0, 0],   # Gornji desni kut
        [2, 1, -1, 0, 0],  # Donji desni kut
        [3, -1, -1, 0, 0], # Donji lijevi kut
        [4, 0, 0, 0, 0]    # Povratak na početak
    ])

    # Broj segmenata između točaka (pieces)
    num_pieces = 4

    # Generiraj trajektoriju
    traj = generate_trajectory(data, num_pieces)

    # Spremi generiranu trajektoriju u datoteku ako je potrebno
    traj.savecsv("square_trajectory.csv")

    print("Trajektorija za kvadrat generirana i spremljena u 'square_trajectory.csv'")
