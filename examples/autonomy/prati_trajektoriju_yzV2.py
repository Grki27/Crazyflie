...
def run_sequence(cf, trajectory_id, duration):
    ...
    time.sleep(height * 5) # sva sleep vremena su relativna, tako samo promjenom visine sve funkcionira glatko
    ...
    x_last, y_last, z_last = absolute_points[len(absolute_points) - 1]
    commander.land(0.0, z_last * 5)
    print("\nTime to land: ", z_last * 5)
    time.sleep(z_last * 5)
    commander.stop()
    ...
    plt.plot(y_positions, z_positions, label="Flight Path")
    # označi posebno početnu i završnu točku
    plt.scatter(y_positions[-1], z_positions[-1], color='red', label="Last Position")
    plt.scatter(start_pos[1], start_pos[2], color='green', label="Start Position", zorder=5)  # Početna pozicija
    plt.scatter(special_y, special_z, color='purple', label=f"Target {i+1} Reached", marker='x')
    plt.title("Flight Path of Crazyflie Drone")
...
if __name__ == '__main__':
    ...
    # problem je što ovo ne funkcionira za nepravilne oblike
    print(absolute_points)
    absolute_points = add_x_coordinate_to_points(absolute_points)
    starting_point = absolute_points[0]
    print("\nAbsolute points 1: \n")
    print(absolute_points)
    absolute_points = adjust_points_to_start_at_takeoff_point(absolute_points, starting_point)
    absolute_points = adjust_points_so_drone_is_above_ground(absolute_points)
    print("\nAbsolute points 2: \n")
    print(absolute_points)
    ...
    file_path = r'C:\Users\izakarij\Desktop\CrazyflieGH\generated_trajectory.csv'
